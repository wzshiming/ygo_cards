...
func d1_1(cardBag *ygo.CardVersion) {
		...
		*/
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterSpellEquip(func(c *ygo.Card) bool {
				...
				c.SetAtk(c.GetAtk() + 300)
				c.SetDef(c.GetDef() + 300)
		...
		*/
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterSpellNormal(func() {
				...
				tar := pl.GetTarget()
				for i := 0; i != 2; i++ {
					if c := pl.SelectForWarnShort(ygo.LO_Removed, 1, tar.Grave(), func(c0 *ygo.Card) bool {
						return c0.IsMonster()
					}); c != nil {
						c.ToRemoved()
					}
	...
	cardBag.Register(&ygo.CardOriginal{
		...
		*/
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterFlip(func() {
				pl := ca.GetSummoner()
				if c := pl.SelectForWarn(ygo.LO_JoinHand, pl.Grave(), func(c0 *ygo.Card) bool {
					return c0.IsTrap()
				}); c != nil {
					c.ToHand()
				}
		...
		*/
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterFlip(func() {
				...
				for cs.Len() != 0 {
					if c := pl.SelectForPopup(ygo.LO_Select, cs); c != nil {
						pl.Deck().EndPush(c)
						cs.PickedFor(c)
					}
				}
				...
				ca.RegisterIgnitionSelector(ygo.SP, func(pl0 *ygo.Player) {
					if pl := ca.GetSummoner(); pl == pl0 {
						...
						css := ygo.NewCards(tar.Mzone(), pl.Mzone(), func(c0 *ygo.Card) bool {
							return c0.IsFaceUp() && (c0.GetAtk() <= 1000) && c0 != ca
						})
						if css.Len() > 1 {
							ca.PushChain(func() {
								ca.Cost(ca)
		...
		*/
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterSpellNormal(func() {
				...
				tar := pl.GetTarget()
				for i := 0; i != 5; i++ {
					if c := pl.SelectForWarnShort(ygo.LO_Removed, 0, tar.Grave(), pl.Grave()); c != nil {
						c.ToRemoved()
					}
				}
			})
		...
		*/
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterAccessArea(ygo.LL_Mzone, func(c *ygo.Card) bool {
				return ca.GetSummoner() == c.GetSummoner() && c.GetName() == "野蛮人1号"
			}, func(c *ygo.Card) {
				ca.SetAtk(ca.GetAtk() + 500)
			}, func(c *ygo.Card) {
				ca.SetAtk(ca.GetAtk() - 500)
			})
			return true
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.AddEvent(ygo.InMzone, func() {
				ca.AddEvent(ygo.InGrave, func() {
					...
					if c := pl.SelectForPopup(ygo.LO_JoinHand, cs); c != nil {
						c.ToHand()
					}
				})
			})
	...
	cardBag.Register(&ygo.CardOriginal{
		...
		*/
		...
		Def:   1300,
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterAccessArea(ygo.LL_Grave, func(c *ygo.Card) bool {
				return ca.GetSummoner() == c.GetSummoner() && c.IsMonster()
			}, func(c *ygo.Card) {
	...
	cardBag.Register(&ygo.CardOriginal{
		...
		*/
		...
		Def:   300,
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterAccessArea(ygo.LL_Hand, func(c *ygo.Card) bool {
				return ca.GetSummoner() == c.GetSummoner()
			}, func() {
				ca.SetAtk(ca.GetAtk() + 300)
				ca.SetDef(ca.GetDef() + 300)
			}, func() {
				ca.SetAtk(ca.GetAtk() - 300)
				ca.SetDef(ca.GetDef() - 300)
			})
	...
	cardBag.Register(&ygo.CardOriginal{
		...
		*/
		...
		Initialize: func(ca *ygo.Card) bool {
			ca.RegisterAccessArea(ygo.LL_Mzone, func(c *ygo.Card) bool {
				return ca.GetSummoner() == c.GetSummoner() && c.GetName() == "野蛮人2号"
			}, func(c *ygo.Card) {
				ca.SetAtk(ca.GetAtk() + 500)
